...
describe('cli: stdin strings', () => {
    it('parseStringsArg: should read JSON from stdin when strings option provided without value', async () => {
        ...
        const result = await parseStringsArg(true, async () =>
    ...
    it('parseStringsArg: should read JSON from stdin when strings option provided as empty string ("")', async () => {
        ...
        const result = await parseStringsArg('', async () =>
            Promise.resolve(stdin),
        );
        expect(result?.kind).toBe('single');
        const translations =
    ...
    it('parseStringsArg: should parse strings when provided as an inline JSON string', async () => {
        ...
        const result = await parseStringsArg(str, async () =>
            Promise.resolve(''),
        );
        expect(result?.kind).toBe('single');
        const translations =
    ...
    it('runAddCommand: should add default-language string when provided via --string', async () => {
        ...
        const content = JSON.parse(await readFile(tempFile, 'utf-8'));
        expect(
            content.strings['greeting-default-only'].localizations.en.stringUnit
                .value,
        ).toBe('Hello');
    });
