...
describe('cli: strings-format integration', () => {
    it('accepts YAML inline when --strings-format yaml', async () => {
        ...
        expect(
            content.strings['greeting-yaml'].localizations.en.stringUnit.value,
        ).toBe('Hello');
        expect(
            content.strings['greeting-yaml'].localizations.ja.stringUnit.value,
        ).toBe('こんにちは');
        expect(
            content.strings['greeting-yaml'].localizations['zh-Hans'].stringUnit
                .value,
        ).toBe('你好，世界.');
    });
    ...
    it('accepts JSON5 inline when --strings-format json', async () => {
        ...
        const content = JSON.parse(await readFile(tempFile, 'utf-8'));
        expect(
            content.strings['greeting-json5'].localizations.en.stringUnit.value,
        ).toBe('Hello');
        expect(
            content.strings['greeting-json5'].localizations.ja.stringUnit.value,
        ).toBe('こんにちは');
        expect(
            content.strings['greeting-json5'].localizations['zh-Hans']
                .stringUnit.value,
        ).toBe('你好，世界.');
    });
});
