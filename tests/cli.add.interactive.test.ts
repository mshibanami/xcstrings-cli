...
describe('cli add: interactive flag', () => {
    ...
    it('routes to interactive handler when flag is set', async () => {
        ...
        const prompt = vi
            .spyOn(interactive, 'captureInteractiveStringsInput')
    ...
    it('supports JSON payload and applies comment from payload', async () => {
        const tempFile = await setupTempFile('no-strings.xcstrings');
        const payload =
            '{ "translations": { "en": "Hi" }, "comment": "Greeting" }';
        const prompt = vi
            .spyOn(interactive, 'captureInteractiveStringsInput')
            .mockResolvedValue(payload);
    ...
    it('prefers CLI comment over payload comment for single interactive payloads and preserves per-language state', async () => {
        const tempFile = await setupTempFile('no-strings.xcstrings');
        const payload =
            '{ "translations": { "en": { "value": "Hi", "state": "new" } }, "comment": "payload-comment" }';
        const prompt = vi
            .spyOn(interactive, 'captureInteractiveStringsInput')
            .mockResolvedValue(payload);
        ...
        try {
            ...
            expect(result).toEqual({
                kind: 'single',
                keys: ['hello.interactive'],
            });
            const content = JSON.parse(await readFile(tempFile, 'utf-8'));
            expect(content.strings['hello.interactive'].comment).toBe(
                'cli-comment',
            );
            expect(
                content.strings['hello.interactive'].localizations.en
                    .stringUnit,
            ).toEqual({ value: 'Hi', state: 'new' });
            expect(
                content.strings['hello.interactive'].localizations.ja,
            ).toBeUndefined();
        } finally {
            prompt.mockRestore();
        }
...
farewell: Farewell comment`;
        ...
        try {
            ...
            expect(
                content.strings.greeting.localizations.en.stringUnit,
            ).toEqual({ value: 'Hello inline', state: 'new' });
            expect(content.strings.greeting.localizations.ja).toBeUndefined();
