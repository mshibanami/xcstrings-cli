...
describe('commands', () => {
    ...
    it('add: should not add languages not present when missingLanguagePolicy is ignore (default)', async () => {
        ...
        expect(
            content.strings.greeting2.localizations.en.stringUnit.value,
        ).toBe('Hello');
        expect(content.strings.greeting2.localizations).not.toHaveProperty(
            'ja',
        );
    });
    ...
    it('add: should add default-language string when provided without translations', async () => {
        ...
        const content = JSON.parse(await readFile(tempFile, 'utf-8'));
        expect(
            content.strings['greeting-default'].localizations.en.stringUnit
                .value,
        ).toBe('Hello');
        expect(content.strings['greeting-default'].extractionState).toBe(
            'manual',
        );
    });
    ...
    it('add: default-language string should override the same language provided in --strings', async () => {
        ...
        const content = JSON.parse(await readFile(tempFile, 'utf-8'));
        expect(
            content.strings['greeting-override'].localizations.en.stringUnit
                .value,
        ).toBe('Hello');
        expect(
            content.strings['greeting-override'].localizations.ja.stringUnit
                .value,
        ).toBe('こんにちは');
    });
    ...
    it('add: should add text to the specified language when provided', async () => {
        ...
        const content = JSON.parse(await readFile(tempFile, 'utf-8'));
        expect(
            content.strings['greeting-ja-text'].localizations.ja.stringUnit
                .value,
        ).toBe('こんにちは');
        expect(
            content.strings['greeting-ja-text'].localizations.en,
        ).toBeUndefined();
    });
    ...
    it('remove: should remove specific languages for a key', async () => {
        ...
        const content = JSON.parse(await readFile(tempFile, 'utf-8'));
        expect(content.strings.closeAction.localizations).not.toHaveProperty(
            'ja',
        );
        expect(content.strings.closeAction.localizations).toHaveProperty('en');
        expect(content.strings.closeAction.localizations).toHaveProperty(
            'zh-Hans',
        );
    });
    ...
    it('remove: removing all languages deletes key and reports removed localizations', async () => {
        ...
        const result = await remove(tempFile, undefined, [
            'en',
            'ja',
            'zh-Hans',
        ]);
